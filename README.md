...
1. **akademik_db.sql**: Skrip SQL untuk membuat database `akademik_db`, tabel (`users`, `students`, `courses`, `takes`), dan data sample.
2. **config.php**: Konfigurasi koneksi database dan fungsi utilitas (proteksi halaman, navigasi dinamis).
3. **login.php**: Form login dengan autentikasi menggunakan `password_verify()`.
4. **logout.php**: Menghapus session dan redirect ke halaman login.
5. **index.php**: Dashboard utama dengan navigasi dinamis berdasarkan role.
6. **courses.php**: Halaman untuk:
   - Admin: Kelola courses (tambah, edit, hapus).
   - Mahasiswa: Melihat daftar courses dan enroll.
7. **students.php**: Halaman untuk Admin mengelola data mahasiswa (tambah, edit, hapus).
8. **reset_passwords.php**: Script sementara untuk reset hash password di database jika login gagal.
...
1. **Setup Database**:
   - Buat database di MySQL:
     ...
     ```
   - Import file `akademik_db.sql`:
     ```bash
     mysql -u root -p akademik_db < akademik_db.sql
     ```
   - Pastikan data tabel (`users`, `students`, `courses`, `takes`) terisi dengan benar.

2. **Konfigurasi Koneksi Database**:
   - Edit `config.php` untuk menyesuaikan kredensial MySQL:
     ```php
     $host = 'localhost';
     $dbname = 'akademik_db';
...
1. **Reset Hash Password**:
   - Simpan file `reset_passwords.php` di folder proyek:
     ```php
     <?php
     // reset_passwords.php
     $host = 'localhost';
...
2. **Verifikasi Data di Database**:
   - Jalankan query berikut di MySQL untuk memeriksa data `users`:
     ```sql
...
4. **Debug Login**:
   - Tambahkan debug di `login.php` untuk melihat detail error:
     ```php
...
2. Akses **Dashboard** (`index.php`): Lihat menu navigasi (Dashboard, Kelola Courses, Kelola Students, Logout).
3. **Kelola Courses** (`courses.php`):
...
4. **Kelola Students** (`students.php`):
...
5. Logout dan verifikasi redirect ke `login.php`.

### Sebagai Mahasiswa
1. Login dengan `mahasiswa` / `mhs123` (atau `sari` / `mhs123`, `andi` / `mhs123`).
2. Akses **Dashboard** (`index.php`): Lihat menu navigasi (Dashboard, Daftar Courses, Logout).
3. **Daftar Courses** (`courses.php`):
...
4. Logout dan verifikasi redirect ke `login.php`.
